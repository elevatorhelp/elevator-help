...
function parseRouterJson(text: string): any {
  ...
  if (firstBrace >= 0 && lastBrace > firstBrace) return JSON.parse(withoutFences.slice(firstBrace, lastBrace + 1));
...
async function callRouterModel(question: string, apiKey: string) {
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash-lite:generateContent?key=${apiKey}`, {
    ...
    body: JSON.stringify({
      contents: [{ parts: [{ text: buildRouterPrompt(question) }] }],
      generationConfig: { responseMimeType: "application/json", temperature: 0 },
    }),
  });
  if (!response.ok) throw new Error(`Gemini router error: ${await response.text()}`);
  const data: any = await response.json();
  const text = data?.candidates?.[0]?.content?.parts?.[0]?.text;
  if (!text) throw new Error("Gemini router returned no content");
  return text as string;
}

function hasKnownAnfahrproblemSignature(question: string) {
  const normalized = question.toLowerCase();
  const english = /pre[-\s]?control/.test(normalized) && /(active|activated|on)/.test(normalized) && /(car|cabin|elevator|lift)/.test(normalized) && /(does\s+not|doesn't|won't|will\s+not|fails?\s+to|not)\s+(start|move|run)/.test(normalized);
  const german = /vorsteuer/.test(normalized) && /aktiv/.test(normalized) && /(fahrkorb|kabine|aufzug)/.test(normalized) && /(fährt\s+nicht\s+an|faehrt\s+nicht\s+an|startet\s+nicht|läuft\s+nicht\s+an|laeuft\s+nicht\s+an)/.test(normalized);
  const persian = /(پیش[‌\s-]?کنترل|پری[‌\s-]?کنترل|vorsteuerung)/i.test(question) && /فعال/.test(question) && /(کابین|آسانسور)/.test(question) && /(راه\s*نمی|حرکت\s*نمی|شروع\s*نمی)/.test(question);
...
function applyKnownSymptomMappings(route: any, question: string) {
  ...
  if ((manufacturer && !manufacturer.includes("new lift")) || (controller && !controller.includes("fst"))) return;
...
function normalizeRoute(route: any, question: string): RouterResult {
  if (typeof route.faultFamily === "string" && route.faultFamily.startsWith("LSU-")) {
    route.faultName = route.faultName ?? route.faultFamily;
    route.faultFamily = "LSU";
  }
  if (route.faultCode === "LSU") { route.faultCode = null; route.faultFamily = "LSU"; }
  if (!route.faultFamily && typeof route.faultName === "string" && route.faultName.startsWith("LSU-")) route.faultFamily = "LSU";
  if (route.faultCode !== null && route.faultCode !== undefined && !/^\d+$/.test(String(route.faultCode))) route.faultCode = null;

  const explicitFaultCodeMatch = question.match(/\b(?:fehler|fault|error|code)?\s*(\d{1,4})\b/i);
  if (explicitFaultCodeMatch && !route.faultCode) route.faultCode = explicitFaultCodeMatch[1];
  const genericFaultNumberOnly = /^\s*(?:fehler|fault|error|code)?\s*\d{1,4}\s*[?.!]*\s*$/i.test(question);
  if (genericFaultNumberOnly) {
    route.manufacturer = null; route.productFamily = null; route.controller = null; route.faultFamily = null; route.faultName = null;
    route.needsClarification = true; route.searchStrategy = "clarify_first"; route.confidence = "low";
    route.clarificationQuestion = route.clarificationQuestion ?? "Which elevator manufacturer or controller is this fault code from?";
  } else applyKnownSymptomMappings(route, question);

  const questionLanguage = route.questionLanguage ?? (/^[\x00-\x7F]*$/.test(question) ? "en" : null);
  return {
    intent: route.intent ?? "unknown",
    questionLanguage,
    preferredSourceLanguage: route.preferredSourceLanguage ?? questionLanguage,
    ...
    normalizedQuestion: typeof route.normalizedQuestion === "string" && route.normalizedQuestion.trim() ? route.normalizedQuestion.trim() : question,
    interpretation: typeof route.interpretation === "string" && route.interpretation.trim() ? route.interpretation.trim() : null,
  };
}

function deterministicStandardsRoute(question: string): RouterResult | null {
  const explicitStandard = /\bEN\s*81\s*[-–]?\s*\d+\b/i.test(question) || /\b(DIN\s*)?(norm|normen|standard|standards)\b/i.test(question) || /(استاندارد|نورم|نُرم)/i.test(question);
  const elevatorDomain = /(aufzug|fahrkorb|kabin(?:e|en)?|kabin\b|schacht(?:wand|tür|grube|kopf)?|schwelle|führungsschiene|gegengewicht|puffer|landing\s+door|shaft(?:\s+wall)?|car\s+sill|cabin|guide\s+rail|counterweight)/i.test(question) || /(آسانسور|کابین|چاه(?:\s*آسانسور)?|دیواره\s*چاه|ریل|وزنه\s*تعادل)/i.test(question);
  const asksNormativeDimension = /(\bmin(?:imum)?\.?\b|\bmax(?:imum)?\.?\b|mindest|höchst|maximal|zulässig|abstand|clearance|distance|dimension|maß|mas+|فاصله|حداقل|حداکثر)/i.test(question);
  if (!(explicitStandard || (elevatorDomain && asksNormativeDimension))) return null;
  const questionLanguage = /[؀-ۿ]/.test(question) ? "fa" : /\b(wer|was|warum|wie|wieviel|wie\s+viel|min|max|abstand|aufzug|fahrkorb|kabin|schacht|schwelle|norm|normen)\b/i.test(question) ? "de" : "en";
  return {
    intent: "standard", questionLanguage, preferredSourceLanguage: questionLanguage,
    manufacturer: null, productFamily: null, controller: null, faultCode: null, faultFamily: null, faultName: null,
    topics: asksNormativeDimension ? ["clearance", "dimensions"] : [], components: [], confidence: "high",
    needsClarification: false, clarificationQuestion: null, searchStrategy: "semantic_broad",
    normalizedQuestion: question, interpretation: null,
  };
}
...
export async function routeQuestion(question: string): Promise<RouterResult> {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try { return normalizeRoute(parseRouterJson(await callRouterModel(question, apiKey)), question); }
    catch (error) { lastError = error; }
  }
  throw lastError instanceof Error ? lastError : new Error("Router failed");
}
